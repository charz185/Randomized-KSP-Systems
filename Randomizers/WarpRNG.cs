...
namespace RandomizedSystems.Randomizers
{
    ...
    public static class WarpRNG
    {
        ...
        {
            rng = new System.Random(WarpDrive.seedString.GetHashCode());
        }
        ...
        {
            if (string.IsNullOrEmpty(WarpDrive.seed2))
            {
                // Can't seed
                return;
            }
            // Reset the index if we exceed the string length
            if (index >= WarpDrive.seed2.Length)
            {
                index = 0;
            }
            // We convert the next character in the seed string to an int
            int nextValue = (int)WarpDrive.seed2[index];
